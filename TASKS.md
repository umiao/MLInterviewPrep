...
#### T-P1-511: T-MLSD-AUDIT-01: Score 10 design problems against L5 framework, produce gap report
- **Priority**: P1
- **Complexity**: M
- **Depends on**: T-P0-510
- **Description**: ## Context
Depends on T-P0-510 (L5 framework + Appendix A Unified Template are now canonical). After the framework is published in id=18, each existing design problem is audited against Appendix A's 11 required sections and 6 quality gates.

**Key context finding**: All 9 classic problems (89-97) share the IDENTICAL 8-heading structure (Overview / Core Concepts / Implementation / Interview Patterns / Comparisons / Key Takeaways / Advanced Topics). Means: (a) current state is uniformly MISSING the L5 skeleton sections, (b) existing ML content becomes nested deep-dive content in the new template, (c) fill tasks for non-worked-example problems can use ONE uniform migration recipe.

## Scope — audit 10 problem nodes
- id=89 Search & Retrieval Systems
- id=90 Recommendation Systems
- id=91 Ads & Click Prediction
- id=92 Marketplace & Logistics (will be upgraded by T-P1-512, but audit CURRENT state)
- id=93 NLP & LLM Systems
- id=94 Computer Vision Systems
- id=95 Fraud & Trust Safety
- id=96 ML Infrastructure Design
- id=97 Generative AI Systems
- id=198 Real-Time Recommendation System Design (will be upgraded by T-P1-513, but audit CURRENT state)

## Deliverables
Write `docs/mlsd_l5_audit_20260418.md`:

```markdown
# MLSD L5 Gap Audit (2026-04-18)

## Methodology
Each problem scored against id=18 Appendix A's 11 Required Sections (0/1/2 each) and 6 Quality Gates (pass/fail). Max score: 22 + 6 = 28.

## Score Matrix
| Node | §1 Clarify | §2 Capacity | §3 Arch | §4 DeepDive | §5 DR | §6 Summary | §Q&A | §SelfCheck | Total/16 | Gates/6 | Grand |
|---|---|---|---|---|---|---|---|---|---|---|---|
| id=89 Search&Retrieval | 0 | 0 | 0 | 2 | 0 | 0 | 1 | 0 | 3/16 | 0/6 | 3/22 |
| ...

## Per-Problem Gap Summary
### id=89 Search & Retrieval Systems (score X/22)
**Has:** Core Concepts (inverted index, BM25), Implementation sketches, Interview Patterns
**Missing:**
- All 6 L5-skeleton sections (Clarification, Capacity, High-Level Arch, DR/Monitoring, Summary, Self-Check)
- Service breakdown by read/write + SLA
- Concrete SLOs (only has "low latency" — needs numbers)
- Deep-dive pseudocode/SQL snippets
**Recommended fill:** USE UNIFORM MIGRATION RECIPE (since all 9 problems share the same starting structure) + per-problem domain specifics (search: inverted-index sharding, query rewrite, BM25 vs BERT retrieval, recall-vs-precision tradeoff)

(repeat for all 10)

## Prioritized Fill Order
Ranked by business relevance × gap size:
1. id=92 Marketplace (T-P1-512 already staged) — Uber final round Apr 27/29
2. id=198 Real-Time Rec (T-P1-513 already staged) — broad applicability
3. id=90 Recommendation Systems — adjacent to 198, high company relevance
4. id=89 Search & Retrieval — Google/Pinterest core
5. id=91 Ads & Click Prediction — Meta/Google relevance
6. id=97 Generative AI Systems — LLM trend, OpenAI/Anthropic relevance
7. id=96 ML Infrastructure Design — L5+ signal topic
8. id=93 NLP & LLM Systems — overlap with 97
9. id=94 Computer Vision Systems — niche unless target company demands
10. id=95 Fraud & Trust Safety — lower priority for user's target roles

## Uniform Migration Recipe (applies to 89/90/91/93/94/95/96/97)
Because all 9 share the same "Overview / Core Concepts / Implementation / Interview Patterns / Comparisons / Key Takeaways / Advanced Topics" skeleton, the migration to L5 template is mechanically:
1. PROMOTE the 7 existing sections into a new "## 4. Deep Dives" umbrella as domain-specific subsections (keep all content)
2. INSERT fresh content for: Prerequisites link / §1 Clarification / §2 Capacity Estimation / §3 High-Level Arch with service-by-SLA / §5 DR & Monitoring with SLOs / §6 Summary / Self-Check
3. Each problem's fresh content is DOMAIN-TAILORED (search needs different Capacity numbers than Ads)
```

## Acceptance Criteria
- [ ] `docs/mlsd_l5_audit_20260418.md` exists and is committed (this IS the deliverable — no DB mutations)
- [ ] All 10 problems scored in the matrix
- [ ] Per-problem gap sections list SPECIFIC missing items (not "weak on X" but "lacks peak-QPS calc in §2")
- [ ] Prioritized fill order references user's target-company context (project_uber_final_round, project_google_interview memories)
- [ ] "Uniform Migration Recipe" section exists since all 9 classics share the same starting skeleton
- [ ] Document is useful as INPUT to Stage 4 fill-task staging (I will read this doc to write the 8 fill task specs)
