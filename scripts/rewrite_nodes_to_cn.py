...
def build_user_prompt(title: str, desc: str) -> str:
    return (
        ...
        "Markdown headings, tables, and list structure exactly. Do NOT add new "
        "sections or new content -- this is a style rewrite, not an expansion. "
        "Return ONLY the rewritten Markdown content, no preface, no wrapping "
        "code fence.\n\n"
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        for i, c in enumerate(candidates, 1):
            ...
            try:
                rewritten, elapsed = call_claude(
                    system_prompt, build_user_prompt(c.title, c.description), model
                )
