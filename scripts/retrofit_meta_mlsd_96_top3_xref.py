"""Retrofit Meta MLSD doc 96 with sd://meta-top3-comments-golden drawer row (T-P0-857).

One surgical insert into company_documents.id=96 ('45min Playbook + 4 Strong
Moments') content (does NOT replace any existing structure, only inserts a
single line):
  ...
  ahead of the cd://94/95/97 sibling block).
...
Self-link contract preserved: cd://96 must remain absent from the Drawer 入口
prepended block.
...
TOP3_GOLDEN_ROW = (
    ...
    "| 第 2 例 45min walkthrough (类比 Reels Golden) "
    "| 对照学 framing/metric/label 三段式 + Strong Moment 编排 |"
)
...
def validate(new_content: str, old_content: str) -> None:
    """Sanity checks mirroring T-P0-857 acceptance criteria."""
    ...
    # AC #2: self-URI cd://96 NOT in Drawer 入口 block.
    ...
    # AC #3: Section 8 (E4 vs E5) — and the rest of the body — preserved.
    # The original Section 8 is "## 8. E4 标准 vs E5 加分上限". Confirm it
    # still exists (the insert is in the prepended Drawer block, not body).
    required_fragments = [
        "<!-- META_MLSD_DRAWER_HEADER_96_20260512 -->",
        ...
        "## 3. 4 Strong Moments",
        "## 8. E4 标准 vs E5 加分上限",
        # Inline narrative reference must survive.
        "[Reels Home Feed (45min walkthrough, 8 段台词 verbatim) →](sd://meta-reels-golden)",
    ]
...
def main() -> int:
    """Retrofit doc 96 with one sd://meta-top3-comments-golden drawer row."""
