...
## 1. DCN v1 / v2 — 显式特征交叉的 Cross Network 路线
...
($\odot$ 是 Hadamard product。)现在每个二阶交叉 $x_i x_j$ 都有独立系数 $W_{ij}$,full-rank 双线性。
...
## 2. DLRM — Meta 2019 工业级 CTR 基线

**核心想法**:稀疏 + 稠密特征分别处理,然后用 dot product 做显式二阶交互,再过 top MLP 出分数。是 FM 思想的深度学习扩展。
...
**为什么这样设计**:dot product 直接对标 FM 的二阶 $\langle v_i, v_j \rangle x_i x_j$,但每个特征用了独立 embedding(FM 是共享 latent factor)。比纯 MLP 暴力混合更结构化,比 DCN 那种 N 阶多项式简单(只到二阶)。
...
**局限**:特征工程不可避免(几百个手工 feature field),scale 起来收益递减——这是后来 HSTU 提出 "DLRM doesn't scale with compute" 的批评。
...
- **Implicit feedback**(点击/观看):
  - ALS-WR:把没观察到的当负样本,但加置信权重,交替优化 user/item 矩阵
  - BPR (Bayesian Personalized Ranking):pairwise loss $\sigma(u^\top v_+ - u^\top v_-)$,直接优化排序

**(c) Neural / Deep CF**

- **NCF** (He et al., 2017):用 MLP 替代 inner product 做 user-item 匹配。后来 Rendle 那篇 "Neural CF vs. dot product" 又指出 dot product 调好其实更强
...
**(b) Early fusion**:projector 把每个模态投到统一维度后直接拼接 + MLP。最常用,但隐式假设 MLP 能学好异构特征的交互——RankMixer 论文恰好质疑这一点。
...
- CE + label smoothing
- Reconstruction + commitment loss(VQ-VAE)
...
- **Negative transfer**:任务冲突时强行共享反而互相拉低。解法:
  ...
  - **MMoE (Ma et al., 2018)**:多个 expert 子网络,每个任务一个 gate 学着挑 expert 的加权组合,允许不同任务用不同的专家
  - **PLE (Tencent, 2020)**:在 MMoE 上区分**任务共享专家**和**任务专属专家**,进一步缓解冲突
...
## 6. RQ-VAE — 把 item 量化成层级语义 ID

**核心想法**:把 item 的连续 embedding 用**层级残差量化**变成一个离散码字元组 $(c_1, c_2, ..., c_N)$,作为 item 的"semantic ID"。这是生成式推荐(TIGER 论文,Google 2023)的关键组件。
...
更糟的是,DLRM、DHEN 这些把所有 field 塞到同一个 interaction module 的做法,会让**高频特征主导,长尾特征被冲掉**——内积的 magnitude 被高方差的特征 dominate。

**RankMixer 的解法**:借鉴 MLP-Mixer 思路但为推荐重新设计:

**(a) Multi-head token mixing**(替代 self-attention)

- Parameter-free 的简单 reshape/transpose 操作做 cross-token interaction
...
- MFU 从 4.5% 提到 45%(传统手工特征交叉模块在 GPU 上是 CPU 时代的 legacy,很多碎算子)
