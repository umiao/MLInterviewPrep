"""Retrofit Meta MLSD doc 95 with sd://meta-top3-comments-golden drawer row (T-P0-856).

One surgical insert into company_documents.id=95 ('Cross-cutting 积木库 (drawer)')
...
Self-link contract preserved: cd://95 must remain absent from the Drawer 入口
prepended block.
...
TOP3_GOLDEN_ROW = (
    ...
    "| 完整 45min walkthrough: framing/metric/label/feature/arch/training/serving/monitoring "
    "| 想看 Top-3 Comments 配套积木实战 |"
)
...
def validate(new_content: str, old_content: str) -> None:
    """Sanity checks mirroring T-P0-856 acceptance criteria."""
    ...
    # AC #2: self-URI cd://95 NOT in Drawer 入口 block.
    ...
    # Body landmarks preserved (untouched by single-row insert).
    required_fragments = [
        "<!-- META_MLSD_DRAWER_HEADER_95_20260512 -->",
...
def main() -> int:
    """Retrofit doc 95 with one sd://meta-top3-comments-golden drawer row."""
