...
- **Complexity**: M
- **Depends on**: None
- **Description**: 把 staging/04_14_ML问题深入拷打.md 13 题压缩成问答卡. Deliverable: docs/google_staging_13_flashcards.md, ingest company_documents (company_id=3, doc_kind=prep_note). 格式: 每题 1 个 Q (自检问题) + 1 段 A (100 字以内, 必含 1 个公式或 1 个数字例子). 不是全文笔记, 是考试卡. AC: 13 张卡 + 末尾 3 个死记口诀 + 面试前晚 checklist (30 分钟过完).
...
#### T-P0-434: [LC] 攻下 85 Maximal Rectangle + 写中文笔记
- **Priority**: P0
- **Complexity**: S
- **Depends on**: None
- **Description**: LC 85 未完成, 用户明确点名考核重点. AC: (1) solve 一次不 peek; (2) 核心解法 = 每行转 histogram, heights[j]+=1 若'1' else 0, 跑 LC 84 单调栈; (3) 时间 O(m·n) 空间 O(n); (4) problems.notes 写中文笔记覆盖: 转化直觉 / 边界哨兵技巧 / 为什么 O(mn) / 和 dp 解法 (left/right/height 三数组) 的对比; (5) 设 is_completed=1.
