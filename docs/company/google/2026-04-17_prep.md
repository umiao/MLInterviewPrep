...
2. **数据分析与处理**
   ...
   - 采样、特征 leakage、train-serve skew (drill: `docs/company/google/train_serve_skew_drill.md`; 涵盖 target encoding K-fold OOF + 平滑、ranking 必须 time-based split、feature store 三种 skew、future-only aggregates、一个 Etsy creator-encoding 真实踩坑)
...
4. **深度问答准备 (high-signal 话题)**
   ...
   - **A/B test**: 样本量、MDE、SRM、novelty effect、CUPED、Etsy GMB trap (drill: `docs/company/google/ab_test_rigor_drill.md`; 基础 note: pillar7.probability_statistics.ab_test_sample_size)
   - **Feature drift / 监控**: PSI、KL、Jensen-Shannon、分阶段 alert (drill: `docs/company/google/feature_drift_drill.md`; 涵盖 PSI 0.1/0.25 阈值、KL vs JS 对称性/有界性、KS 连续特征、covariate vs concept vs label shift、per-feature + per-segment alerting)

### 快速复习 pointer
- `docs/company/doordash/ml_domain_ranking.md` — 排序损失 + eval
- `docs/company/doordash/ml_domain_features_dl.md` — 特征工程 + DL 基础
- `docs/company/doordash/ml_domain_fundamentals.md` — bias/variance, regularization
- `docs/company/doordash/ml_domain_case_study.md` — 完整 ML case 结构
