"""Seed DoorDash ML Domain Search + Semantic Matching + Bias/Debiasing prep doc into company_documents."""
import os
import sqlite3
...
DOC_TITLE = "DoorDash ML Domain Prep: Search + Semantic Matching + Bias/Debiasing"


def main() -> None:
    """Read the markdown doc and upsert into company_documents."""
    doc_path = os.path.join(DOCS_DIR, "doordash_ml_domain_search.md")
    ...
    conn.commit()
    conn.close()
    print("[DONE] DoorDash search + semantic matching + bias/debiasing prep doc seeded successfully")
