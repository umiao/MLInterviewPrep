"""Seed: T-P0-701 [MLI-GOLDEN-LR] -- Replace problems.id=1102 notes with the
condensed Linear Regression golden draft.

Targets problems.id=1102 ("Meta AI-Native Coding - Linear Regression
(closed-form X^TX + Ridge/Lasso/SGD follow-ups)"). Mirrors
scripts/seed_kmeans_golden_v1.py: REPLACES the entire notes column with the
rewrite at docs/drafts/lr_golden_v1.md (per the meta-prompt at
docs/methodology/ml_impl_note_rewrite_spec.md).

Length target (per task spec):
- Baseline notes: 9,894 chars.
- Cap: <= 6,900 chars (>= 30% reduction).
- Current draft body + sentinel: ~5,384 chars (45.6% reduction).

Idempotency:
- Sentinel `<!-- LR_GOLDEN_V1_20260502 -->` prepended as the first line.
...
DRAFT_PATH = REPO_ROOT / "docs" / "drafts" / "lr_golden_v1.md"
PROBLEM_ID = 1102
SENTINEL = "<!-- LR_GOLDEN_V1_20260502 -->"
LENGTH_CAP = 6900
