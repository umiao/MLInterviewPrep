...
"""Seed DoorDash ML Domain case study mock answers + SCOPE templates into company_documents."""
import os
import sqlite3
...
DOC_TITLE = "DoorDash ML Domain Prep: Case Study Mock Answers + SCOPE Templates"


def main() -> None:
    """Read the markdown doc and upsert into company_documents."""
    doc_path = os.path.join(DOCS_DIR, "doordash_ml_domain_case_study.md")
    ...
    conn.commit()
    conn.close()
    print(f"[DONE] DoorDash case study prep doc seeded successfully ({len(content)} chars)")
