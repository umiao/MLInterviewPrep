...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 994 Rotting Oranges | 多源 BFS | 所有腐烂橘子同时开始 | O(mn) | O(mn) |
| LC 1020 Number of Enclaves | 边界 BFS | 从边界 flood-fill，计数剩余 | O(mn) | O(mn) |
| LC 1197 Min Knight Moves | 带剪枝的 BFS | 对称性：仅在第一象限工作 | O(xy) | O(xy) |
| LC 815 Bus Routes | 路线图上的 BFS | 节点 = 路线，边 = 共享站点 | O(sum routes) | O(sum routes) |
| LC 2503 Max Grid Points | BFS + 排序查询 | 按升序处理查询，扩展前沿 | O(mn log mn) | O(mn) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 79 Word Search | 网格 **DFS (Depth-First Search，深度优先搜索)** + 回溯 | 用 '#' 原地标记已访问，恢复 | O(mn * 3^L) | O(L) |
| LC 79 variant: 8-dir straight | 线性扫描，无回溯 | 每个起点一个方向——简单得多 | O(mn * 8 * L) | O(1) |
| LC 17 Letter Combos | 回溯 | 数字映射到字符，枚举 | O(4^n * n) | O(n) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 230 Kth Smallest BST | 中序遍历 | 迭代中序，在第 k 个停止 | O(H+k) | O(H) |
| LC 230 variant: Kth Largest | 反向中序 | Right -> root -> left | O(H+k) | O(H) |
| LC 230 follow-up: Morris | O(1) 空间遍历 | 线索化前驱节点 | O(n) | O(1) |
| LC 230 follow-up: Augmented | 存储子树大小 | 通过 left_count 实现 O(H) 查找 | O(H) | O(1) |
| LC 337 House Robber III | 树形 **DP (Dynamic Programming，动态规划)** (取/跳) | 每个节点返回 (rob, skip) 对 | O(n) | O(H) |
| LC 549 Longest Consecutive II | 树形 DP (递增/递减) | 同时追踪递增和递减长度 | O(n) | O(H) |
| LC 987 Vertical Traversal | **BFS (Breadth-First Search，广度优先搜索)** + 列追踪 | 按 (col, row, val) 排序 | O(n log n) | O(n) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 547 Number of Provinces | 基础 UF 或 DFS | 计数不同的根 | O(n^2 alpha(n)) | O(n) |
| LC 1697 Edge Length Limited | 离线查询 + **UF (Union Find，并查集)** | 按权重排序边和查询 | O((E+Q) log) | O(n+Q) |
| LC 1697 variant: weight >= k | 反向排序 | 按降序处理边权重 | O((E+Q) log) | O(n+Q) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 981 TimeMap | 时间戳上的 **BS (Binary Search，二分查找)** | 在有序时间戳列表上 bisect_right | O(log n) get | O(n) |
| LC 977 Squares Sorted (related) | 有序输入 | 双指针比 BS 更好 | O(n) | O(n) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 1696 Jump Game VI | DP + 单调双端队列 | 滑动窗口最大值实现 O(n) DP 转移 | O(n) | O(k) |
| LC 1696 variant: prime jumps | DP + 筛法 | 预计算以3结尾的素数作为跳跃大小 | O(n*P) | O(n) |
| LC 2858 Min Edge Reversals | 换根 DP | 计算根0，每条边 +1/-1 传播 | O(n) | O(n) |
| Custom #8 Robot Distance Grid | DP 预计算 | 通过网格 DP 预计算距离 | O(mn) | O(mn) |
| Custom #18 Jump Game Prime | DP + 筛法 | 跳 +1 或 +以3结尾的素数 | O(n*P) | O(n) |
| Custom #19 Min Edge Reversal | 换根 DP | 同 LC 2858 | O(n) | O(n) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 23 Merge K Sorted Lists | k 个头节点的最小堆 | 弹出最小值，推入其下一个 | O(N log k) | O(k) |
| LC 23 variant: divide & conquer | 两两合并 | 反复合并对 | O(N log k) | O(1) |
...
| LC 1696 (双端队列方面) | 滑动窗口最大值 | 单调双端队列维护窗口内最大值 | O(n) | O(k) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 977 Squares of Sorted Array | 从两端双指针 | 最大绝对值在边缘 | O(n) | O(n) |
...
| 题目 | 变体 | 关键思路 | 时间 | 空间 |
|------|------|----------|------|------|
| LC 2791 Palindrome Paths | XOR 前缀位掩码 | 路径 u->v 回文当且仅当 XOR 最多1位为1 | O(26n) | O(n) |
...
是在树上吗？
  是 -> 是 BST 的第 k 小/搜索？
           是 -> 中序遍历 (LC 230)
         是"取或跳"节点决策？
           是 -> 树形 DP (LC 337, LC 549)
         是"所有根的最优解"？
           是 -> 换根 DP (LC 2858, #19)
         是"回文路径"？
           是 -> 位掩码 XOR (LC 2791, #20)
         其他 -> 树 DFS (#14)

是在图上吗？
  是 -> 无权"最短路径"？
           是 -> BFS (LC 994, LC 1197, #22, #23)
         "连通分量"或"同一组"？
           是 -> Union Find (LC 547, LC 1697, #3)
         "公交线路数/换乘"？
           是 -> 路线图 BFS (LC 815)
...
| 问题中的信号 | 模式 | 示例 |
|-------------|------|------|
| "最短路径"、"最少步数" | BFS | LC 994, LC 1197 |
| "连通分量"、"同一组" | Union Find | LC 547, LC 1697 |
| "BST 中第 k 小/大" | 中序遍历 | LC 230 |
| "树上取或跳" | 树形 DP | LC 337 |
| "所有根最优" | 换根 DP | LC 2858 |
| "回文路径 + 树" | 位掩码 XOR | LC 2791 |
