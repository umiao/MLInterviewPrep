...
- **Complexity**: M
- **Depends on**: None
- **Description**: src/backend/models/scraper.py. SeedURL: url UNIQUE, source_site CheckConstraint. ScrapedPage: UniqueConstraint(url,content_hash). InterviewQuestion: question_type CheckConstraint, mapped_framework_node_id FK. AC: duplicate url/hash raise IntegrityError. Test: tests/test_models_scraper.py.
...
- **Complexity**: S
- **Depends on**: None
- **Description**: Create problem. Duplicate leetcode_id -> 409. Null leetcode_id always OK. Convert tags/company_tags to JSON. Return 201. AC: create returns id, duplicate leetcode_id 409. Depends: T-P0-9.
...
- **Complexity**: M
- **Depends on**: None
- **Description**: POST: create attempt, update problem (last_attempted_at, comfort_level, is_completed sticky). GET: list newest first. AC: comfort_after>=3 sets is_completed=True, stays True. Depends: T-P0-11.
