...
DESCRIPTION = (
    ...
    "Logistic Regression -- 单样本 logit $z = w^\\top x + b$, $p = \\sigma(z) = "
    "1/(1+e^{-z})$, Bernoulli MLE 等价最小化 BCE; 解析梯度 "
    "$\\nabla_w L = \\frac{1}{n} X^\\top (\\sigma(Xw) - y)$ 由 sigmoid + CE "
    "的“漂亮消去”直接给出; full-batch gradient descent 训练循环. "
    "同时给出 softmax / cross-entropy 的多分类扩展 ($W$ 形状 $(d, K)$, "
    "梯度 $\\frac{1}{n} X^\\top (P - Y)$) 与 L1/L2 正则化在梯度上的修正.\n\n"
    "核心讨论点: (1) sigmoid + CE 的“漂亮消去”: $\\sigma'(z) = "
    "\\sigma(z)(1 - \\sigma(z))$ 被 $\\partial \\ell / \\partial p$ 里的 $1/p$, "
    "$1/(1-p)$ 精确抵消, 单样本梯度塌缩到 $(p - y)\\, x$, 与 Linear Regression "
    "MSE 同形 -- GLM 框架; (2) 为什么用 BCE 不用 MSE: BCE + sigmoid 凸 + "
    "梯度饱满 (错得越离谱越饱满), MSE + sigmoid 非凸 + 梯度饱和 ($p(1-p) \\to 0$); "
    "(3) 凸优化与无闭式解: Hessian $\\frac{1}{n} X^\\top \\mathrm{diag}(p(1-p)) X "
    "\\succeq 0$, GD / Newton / L-BFGS 全局收敛, 但 sigmoid 让一阶条件非线性, "
    "没 $(X^\\top X)^{-1}$ 那种解析解; (4) **数值稳定性 (logits-space, 工业灵魂)**: "
    "朴素 BCE 大 $|z|$ 处 $\\log(0)$ 爆 `nan`, 改写到 logits 空间得 "
    "$L = \\max(z, 0) - z y + \\log(1 + e^{-|z|})$ -- 这是 PyTorch "
    "`binary_cross_entropy_with_logits` / TF `sigmoid_cross_entropy_with_logits` "
    "的实现核心; (5) 类别不平衡: 调阈值 / pos_weight / focal loss; "
    "(6) Newton / IRLS 与 GD 的关系, 何时换 quasi-Newton (L-BFGS); "
    "(7) calibration: Platt scaling vs isotonic regression; "
    "(8) L2 ↔ MAP + Gaussian prior (各 $w_j$ shrinkage), "
    "L1 ↔ Laplace prior (稀疏解, 自带特征选择)."
)
